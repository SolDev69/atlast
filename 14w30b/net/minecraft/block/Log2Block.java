...
public class Log2Block extends AbstractLogBlock {
   ...
   public void addToCreativeMenu(Item item, ItemGroup group, List stacks) {
      stacks.add(new ItemStack(item, 1, PlanksBlock.Variant.ACACIA.getIndex() - 4));
      stacks.add(new ItemStack(item, 1, PlanksBlock.Variant.DARK_OAK.getIndex() - 4));
   }
   ...
   public int getDropItemMetadata(BlockState state) {
      return ((PlanksBlock.Variant)state.get(VARIANT)).getIndex() - 4;
   }
}
